...
int main(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, "A:B:b:D:dF:hM:m:n:p:R:r:S:s:T:t:V:vx:")) !=
         -1) {
    switch (ch) {
    ...
    case 'p': /* Proxy mode */
      ...
      break;
    case 'R': /* Retry lookup */
      sdt_dns_setopt(SDT_RES_RETRY, sdt_strtonum(optarg, 0, INT32_MAX));
      break;
    case 'r':
    ...
    case 's': /* forwarded session */
      ...
      break;
    case 'T':
      sdt_dns_setopt(SDT_RES_USEVC, sdt_strtonum(optarg, 0, INT32_MAX));
      break;
    ...
    case 'x': /* Transmit lookup timeout */
      sdt_dns_setopt(SDT_RES_RETRANS, sdt_strtonum(optarg, 0, INT32_MAX));
      break;
...
void sdt_loop_A(SDT_STATE *ss) {
  ...
  while ((n = sdt_read(ss, buf, ss->bufsz)) > 0) {
    ...
    VERBOSE(3, "A record: res timeout = %d, sleep = %f seconds\n", ss->backoff,
            (float)(ss->sleep * ss->backoff) / 1000000);
    usleep(ss->sleep);
  }
...
void usage(SDT_STATE *ss) {
  ...
  (void)fprintf(stderr, "-A                       Delay A queries to force "
                        "full buffer reads [default: %d microseconds]\n",
                ss->delay);
  (void)fprintf(stderr, "-B                       Size of read buffer (A "
                        "queries) [default: %d bytes]\n",
                (u_int32_t)ss->bufsz);
  (void)fprintf(stderr, "-b                       Maximum backoff for polling "
                        "server [default: %d]\n",
                ss->maxbackoff);
  (void)fprintf(stderr,
                "-D <host>:<port>         Dynamically forward a session\n");
  (void)fprintf(stderr, "-F <num>                 Fast start, number of small "
                        "packets to pass w/out buffering (0 to disable) "
                        "[default: %d]\n",
                ss->faststart);
  (void)fprintf(stderr, "-M                       Maximum number of polling "
                        "query failures [default: %d]\n",
                ss->maxpollfail);
  (void)fprintf(stderr, "-m                       Minimum time to sleep "
                        "between nameserver queries [default: %d us]\n",
                ss->sleep);
  (void)fprintf(stderr, "-n <roundrobin|random>   Strategy for shuffling "
                        "domain names [default: roundrobin]\n");
  (void)fprintf(stderr, "-p <port>                proxy port, listen on a TCP "
                        "port instead of using stdin/stdout\n");
  (void)fprintf(stderr,
                "-R <number>              Number of retries for lookup\n");
  (void)fprintf(stderr, "-r <namserver>           Nameserver (or keyword: "
                        "random, opendns, level3, speakeasy, google)\n");
  (void)fprintf(stderr, "-S [rotate|blast]        Resolver strategy\n");
  (void)fprintf(stderr, "-s <number>              Forward session\n");
  (void)fprintf(stderr, "-T <number>              Use TCP [0 = new connection "
                        "for each request, 1 = pipeline requests]\n");
  (void)fprintf(stderr,
                "-t <DNS type>            TXT, CNAME, NULL [Default = TXT]\n");
