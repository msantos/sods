...
int sdt_dns_setns(char *ns) {
  ...
  (void)memcpy(&_res.nsaddr_list[nx++].sin_addr, he->h_addr_list[0],
               he->h_length);
  _res.nscount = nx;
...
int sdt_dns_A(SDT_STATE *ss, char *buf, ssize_t n) {
  ...
  switch (ss->protocol) {
  case PROTO_DYN_FWD:
    (void)snprintf(query, sizeof(query), "%s.%u-%u.id-%u.u.%s-%u.x.%s", dn,
                   nonce, (u_int32_t)ss->sum_up, htonl(ss->sess.id), ss->target,
                   ss->target_port, ss->dname_next(ss));
    break;
  default:
    /* Create the domain name:
     *  $temp_payload.$nonce-$sum_up.id-$id.up.$extension
     */
    (void)snprintf(query, sizeof(query), "%s.%u-%u.id-%u.up.%s", dn, nonce,
                   (u_int32_t)ss->sum_up, htonl(ss->sess.id),
                   ss->dname_next(ss));
    break;
  }
...
char *sdt_dns_poll(SDT_STATE *ss, ssize_t *len) {
  ...
  case PROTO_DYN_FWD:
    (void)snprintf(query, sizeof(query), "%u-%u.id-%u.d.%s-%u.x.%s",
                   (u_int32_t)ss->sum, nonce, htonl(ss->sess.id), ss->target,
                   ss->target_port, ss->dname_next(ss));
    break;
  default:
    ...
    (void)snprintf(query, sizeof(query), "%u-%u.id-%u.down.%s",
                   (u_int32_t)ss->sum, nonce, htonl(ss->sess.id),
                   ss->dname_next(ss));
    break;
  }
  ...
  *len = 0;
  if ((buflen = res_search(query, ns_c_in, ss->type, (u_char *)&pkt,
                           sizeof(pkt))) < 0) {
...
int sdt_dns_parsens(SDT_STATE *ss, char *buf) {
  ...
  if (strcasecmp(serv, "random") == 0) {
    for (i = 0; i < MAXNS; i++) {
      ds = dnsserv + (sdt_arc4random(ss->rand) %
                      ((sizeof(dnsserv) / sizeof(SDS_SERV)) - 1));
      (void)sdt_dns_setns(ds->addr);
      VERBOSE(1, "Using %s (%s) = %s\n", ds->name, ds->descr, ds->addr);
    }
