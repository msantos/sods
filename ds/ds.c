...
void ds_writer(DS_STATE *ds) {
  ...
  if (res_mkquery(ns_o_query, DS_HOSTNAME, ns_c_in, ns_t_a, NULL, 0, NULL, buf,
                  NS_PACKETSZ) < 0)
  ...
  for (ip = ds->first; ip <= ds->last; ip++) {
    ...
    if (sendto(ds->s, buf, NS_PACKETSZ, 0, (struct sockaddr *)&ds->sa,
               sizeof(ds->sa)) < 0)
  ...
  for (;;) {
    ...
    if ((hdr.dns_flags & 0x8000) == 0) {
      warnx("not a response packet: %s:%d = %u\n", inet_ntoa(sa.sin_addr),
            ntohs(sa.sin_port), hdr.dns_flags);
      continue;
    }
