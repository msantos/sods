...
static long long sds_strtonum(SDS_STATE *ss, const char *nptr, long long minval,
                              long long maxval);
...
int main(int argc, char *argv[]) {
  ...
  while ((ch = getopt(argc, argv, "c:Dd:g:hi:L:p:u:vx:")) != -1) {
    ...
    case 'L': /* <session id>:<remote address>:<remote port> */
      ...
      break;
    case 'p':
      ss->local.sin_port = htons(sds_strtonum(ss, optarg, 0, 0xfffe));
      break;
    case 'u':
...
void usage(SDS_STATE *ss) {
  ...
  (void)fprintf(stderr, "usage: sods <option> <domain name>\n"
                        "             -c     max connections\n"
                        "             -D     run as a daemon\n"
                        "             -d     chroot directory\n"
