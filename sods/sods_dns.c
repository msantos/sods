...
int sds_dns_type(SDS_PKT *pkt) {
  ...
  if ((p = memchr(pkt->data.query, '\0',
                  pkt->datalen - sizeof(pkt->data.hdr) - 1)) == NULL)
    return 0;
...
int sds_dns_getdn(SDS_STATE *ss, SDS_PKT *pkt) {
   ...
   * additional records.
   */
  if ((pkt->data.hdr.no_additional > 0) || (pkt->data.hdr.no_answers > 0) ||
      (pkt->data.hdr.no_authority > 0)) {

    VERBOSE(3, "Truncating query: no_additional = %d, no_answers = %d, "
               "no_authority = %d\n",
            htons(pkt->data.hdr.no_additional), htons(pkt->data.hdr.no_answers),
            htons(pkt->data.hdr.no_authority));

    if ((p = memchr(pkt->data.query, '\0',
                    pkt->datalen - sizeof(pkt->data.hdr) - 4)) == NULL)
      return -1;

    pkt->datalen = sizeof(pkt->data.hdr) + strlen((char *)pkt->data.query) +
                   5; /* 2 * sizeof(u_int16_t): NS type, NS class */
...
int sds_dns_query_A(SDS_STATE *ss, SDS_PKT *pkt) {
  ...
  *p++ = '\0';
  ...
  if (sscanf(p, "%u.id-%u.up.%32s", (u_int32_t *)&pkt->sum_up, &pkt->sess.id,
             domain) != 3)
    goto ERR;
...
int sds_dns_query_TXT(SDS_STATE *ss, SDS_PKT *pkt) {
  ...
  if (sscanf(pkt->buf, "%u-%u.id-%u.down.%32s", (u_int32_t *)&pkt->sum, &nonce,
             &pkt->sess.id, domain) != 4)
    return -1;
...
ssize_t sds_dns_enc_TXT(SDS_STATE *ss, SDS_PKT *pkt) {
  ...
  for (len = 0; len < pkt->buflen; len += BUFLEN) {
    ...
    if (b64_ntop((u_char *)pkt->buf + len, s, txt.data, sizeof(txt.data)) < 0)
      errx(EXIT_FAILURE, "sds_dns_enc_TXT: invalid input: %zu/%zu", s,
           sizeof(txt.data));
...
ssize_t sds_dns_enc_NULL(SDS_STATE *ss, SDS_PKT *pkt) {
  ...
  if (b64_ntop((u_char *)pkt->buf, pkt->buflen, b64, sizeof(b64)) < 0)
    errx(EXIT_FAILURE, "sds_dns_enc_NULL: invalid input: %zu/%zu", len,
         sizeof(b64));
